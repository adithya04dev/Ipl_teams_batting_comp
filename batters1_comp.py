...
class Batter_comp():
    ...
    def calculate(self,overs1,BowlingType,Season,limit):
            ...
            players=self.df.loc[(self.df['Season'].isin(Season)) & (self.df["BowlingType"].isin(BowlingType)) & (self.df["overs"].isin(overs)) ]['BattingTeam'].unique()
            
            for player in players:
            
                run = int(self.df.loc[(self.df["BattingTeam"] == player)  & (self.df["BowlingType"].isin(BowlingType)) & (self.df["overs"].isin(overs)) & (self.df["Season"].isin(Season))].batsman_run.sum())
                ...
                balls=len(self.df.loc[(self.df['extra_type']!="wides")  & (self.df["BowlingType"].isin(BowlingType)) & (self.df["overs"].isin(overs))  & (self.df["Season"].isin(Season)) & (self.df['extra_type']!="noballs") & (self.df['BattingTeam'] == player) ] )
                out = len(   self.df.loc[(self.df["BattingTeam"] == player) & (self.df["player_out"].notnull())  & (self.df["BowlingType"].isin(BowlingType))   & (self.df["overs"].isin(overs)) & (self.df["Season"].isin(Season))])
                boundary = len(self.df.loc[(self.df["BattingTeam"] == player) & ((self.df["batsman_run"] == 4) | (self.df["batsman_run"] == 6)  ) & (self.df["BowlingType"].isin(BowlingType)) & (self.df["overs"].isin(overs)) & (self.df["Season"].isin(Season))])
                dots=len(self.df.loc[(self.df["BattingTeam"] == player)  & (self.df["Extras_Run"]==0) & (self.df["batsman_run"]==0) & (self.df["BowlingType"].isin(BowlingType)) & (self.df["overs"].isin(overs))  & (self.df["Season"].isin(Season))  ] )
...
#r1=batcomp.calculate([1],['RAP','LAP'],[2023],40)
#print(r1)
